...
/*
    ...
    most users won't use 4A so badge will run lower than this so use 3.5V as
   minimum.
*/
...
static mp_obj_t power_SendVDM(mp_obj_t app_id_in, mp_obj_t data_in) {
  uint16_t app_id = (uint16_t)(mp_obj_get_int(app_id_in));

  uint8_t data[24] = {0};
  int data_index = 0;

  mp_obj_iter_buf_t iter_buf;
  mp_obj_t item, iterable = mp_getiter(data_in, &iter_buf);
  while (((item = mp_iternext(iterable)) != MP_OBJ_STOP_ITERATION) && (data_index < 24)) {
    data[data_index] = (uint8_t)(mp_obj_get_int(item));
    data_index += 1;
  }

  tildagon_send_vdm(app_id, data);
  return MP_OBJ_NEW_SMALL_INT(data_index);
}

static MP_DEFINE_CONST_FUN_OBJ_2(SendVDM_obj, power_SendVDM);
...
static mp_obj_t power_BatteryLevel(void) {
  ...
  if (((pmic.status & BQ_CHARGE_STAT_MASK) == BQ_NOTCHARGING) ||
    ...
    level =
        ((pmic.vbat - VBAT_DIS_MIN) / (VBAT_DIS_MAX - VBAT_DIS_MIN)) * 100.0F;
  } else {
    ...
    if (pmic.vbat < VBAT_CHG_MAX) {
      level = ((pmic.vbat - VBAT_CHG_MIN) / (VBAT_CHG_MAX - VBAT_CHG_MIN)) *
              vbat_ci_percent;
    } else {
      level =
          100.0F - ((pmic.ichrg / (max_current - IBAT_TERM)) * vbat_cv_percent);
    }
  }
  if (level > 100.0F) {
    level = 100.0F;
...
static MP_DEFINE_CONST_FUN_OBJ_0(BatteryChargeState_obj,
                                 power_BatteryChargeState);
  ...
  if (usb_in.pd.number_of_pdos > 0) {
    for (uint8_t i = 0; i < usb_in.pd.number_of_pdos; i++) {
      ...
      case 0: /* fixed supply */
      {
        ...
        tuple[2] =
            mp_obj_new_float((usb_in.pd.pdos[i].fixed.voltage * 50) / 1000.0F);
      ...
      case 1: /* battery */
      {
        ...
        tuple[1] = mp_obj_new_float((usb_in.pd.pdos[i].battery.min_volt * 50) /
                                    1000.0F);
        tuple[2] = mp_obj_new_float((usb_in.pd.pdos[i].battery.max_volt * 50) /
                                    1000.0F);
        mp_obj_t capability = mp_obj_new_tuple(3, tuple);
        mp_obj_list_append(capabilities, capability);
        break;
      ...
      case 2: /* non battery variable supply */
      {
        ...
        tuple[2] = mp_obj_new_float(
            (usb_in.pd.pdos[i].variable.max_voltage * 50) / 1000.0F);
        mp_obj_t capability = mp_obj_new_tuple(3, tuple);
        mp_obj_list_append(capabilities, capability);
        break;
      }
...
static MP_DEFINE_CONST_FUN_OBJ_0(SupplyCapabilities_obj,
                                 power_SupplyCapabilities);

static mp_obj_t power_Fault(void) {
  mp_obj_t faults = mp_obj_new_dict(3);
  if (pmic.fault & BQ_FAULT_BATTERY_MASK) {
    mp_obj_dict_store(faults, mp_obj_new_str("Battery", 7),
                      mp_obj_new_str("Battery Over Voltage", 20));
  } else {
    mp_obj_dict_store(faults, mp_obj_new_str("Battery", 7),
                      mp_obj_new_str("Normal", 6));
  }
  if (pmic.fault & BQ_FAULT_BOOST_MASK) {
    mp_obj_dict_store(faults, mp_obj_new_str("Boost", 5),
                      mp_obj_new_str("Overloaded or low battery", 25));
  } else {
    mp_obj_dict_store(faults, mp_obj_new_str("Boost", 5),
                      mp_obj_new_str("Normal", 6));
  }
  ...
  case BQ_FAULT_INPUT: {
    mp_obj_dict_store(faults, mp_obj_new_str("Charge", 6),
                      mp_obj_new_str("Input Fault", 11));
    break;
  }
  case BQ_FAULT_TIMER: {
    mp_obj_dict_store(faults, mp_obj_new_str("Charge", 6),
                      mp_obj_new_str("Safety timer expired", 20));
    break;
  }
...
static const mp_rom_map_elem_t power_module_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_SendVDM), MP_ROM_PTR(&SendVDM_obj)},
    ...
    {MP_ROM_QSTR(MP_QSTR_BatteryChargeState),
     MP_ROM_PTR(&BatteryChargeState_obj)},
    {MP_ROM_QSTR(MP_QSTR_SupplyCapabilities),
     MP_ROM_PTR(&SupplyCapabilities_obj)},
