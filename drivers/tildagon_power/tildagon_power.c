...
void tildagon_power_fast_task(void *param __attribute__((__unused__))) {
  ...
  // turn off 5V switch before setting up PMIC as the reset will enable the
  // boost.
...
void tildagon_power_init(void) {
  xTaskCreatePinnedToCore(tildagon_power_fast_task, "power_task", 2048, NULL,
                          tskIDLE_PRIORITY + 3, &tildagon_power_task_handle, 0);
}
 ...
 */
void tildagon_send_vdm(uint16_t app_id, uint8_t data[24]) {
  fusbpd_vendor_specific(&usb_out.pd, app_id, data);
...
void host_disabled_handler(event_t event) {
  if (event == HOST_TOGGLE) {
    ...
    usb_out.fusb.cc_select =
        (usb_out.fusb.statusa >> FUSB_STATUSA_TOGGLE_SHIFT) & 0x03;
...
void host_unattached_handler(event_t event) {
  if (event == HOST_ATTACH) {
    ...
    fusbpd_vendor_specific_badge_id(&usb_out.pd);
    fusb_send(&usb_out.fusb, usb_out.pd.tx_buffer, usb_out.pd.message_length);
...
void device_disabled_handler(event_t event) {
  if (event == DEVICE_TOGGLE) {
    ...
    usb_in.fusb.cc_select =
        (usb_in.fusb.statusa >> FUSB_STATUSA_TOGGLE_SHIFT) & 0x03;
...
void device_unattached_handler(event_t event) {
  if (event == DEVICE_ATTACH) {
    ...
    if ((usb_in.fusb.input_current_limit >= 1500) &&
  ...
  case WAITING: {
    if (event == DEVICE_GOODCRCSENT) {
      ...
      if (usb_in.pd.last_rx_data_msg_type == PD_DATA_SOURCE_CAPABILITIES) {
        /*
            We only need 5V so can use the first object, from the usb 3
           standard: The vSafe5V Fixed Supply Object Shall always be the first
           object. A Source Shall Not offer multiple Power Data Objects of the
           same type (fixed, variable, Battery) and the same Voltage but Shall
        ...
        */
        ...
        if (usb_in.pd.vdm.app_id != 0xFFFF) {
            if (usb_in.pd.vdm.app_id != 0x0000) {
              push_event(MP_POWER_EVENT_VENDOR_MESSAGE);
            }
            device_pd_state = BADGE_TO_BADGE;
        }
        usb_in.pd.last_rx_data_msg_type = PD_DATA_DO_NOT_USE;
      }
    }
...
void generate_events(void) {
  ...
  while (gpio_get_level(GPIO_NUM_10) == 0) {
    ...
    if (gpio_get_level(GPIO_NUM_10) == 0) {
      ...
      if ((interrupt & FUSB_VBUSOK_I_MASK) &&
          (usb_in.fusb.status & FUSB_STATUS_VBUSOK_MASK)) {
        const event_t event = DEVICE_ATTACH;
        xQueueSendToBack(event_queue, (void *)&event, (TickType_t)0);
      } else if ((interrupt & FUSB_VBUSOK_I_MASK) &&
                 ((usb_in.fusb.status & FUSB_STATUS_VBUSOK_MASK) == 0)) {
    ...
    if (gpio_get_level(GPIO_NUM_10) == 0) {
      ...
      if ((((usb_out.fusb.status & FUSB_STATUS_COMP_MASK) == 0U) &&
           (interrupt & FUSB_CMPCHG_I_MASK)) ||
          ((interrupt & FUSB_BC_LVL_I_MASK) &&
           ((usb_out.fusb.status & FUSB_STATUS_BCLVL_MASK) < 3))) {
        const event_t event = HOST_ATTACH;
        xQueueSendToBack(event_queue, (void *)&event, (TickType_t)0);
      } else if ((usb_out.fusb.status & FUSB_STATUS_COMP_MASK) &&
                 (interrupt & FUSB_CMPCHG_I_MASK)) {
  ...
  if (bc_level > 0) {
    if (bc_level == 2) {
      state->fusb.input_current_limit = 1500U;
               ...
               ((state->fusb.status & FUSB_STATUS_COMP_MASK) == 0)) {
...
void clean_in(void) {
  ...
  *((uint32_t *)&usb_in.pd.rx_badge_id[4]) = 0U;
  usb_in.pd.vdm.app_id = 0U;
  for (int i = 0; i < 24; i++) {
    usb_in.pd.vdm.data[i] = 0U;
  }
}
...
void clean_out(void) {
  ...
  *((uint32_t *)&usb_out.pd.rx_badge_id[4]) = 0U;
  ...
  usb_out.pd.vdm.app_id = 0U;
  for (int i = 0; i < 24; i++) {
    usb_out.pd.vdm.data[i] = 0U;
  }
