...
static mp_obj_t get_fps() { return mp_obj_new_float(smoothed_fps); }
...
static FramebufferState fb_sect_state[4] = {FB_INVALID, FB_INVALID, FB_INVALID,
                                            FB_INVALID};
  ...
  for (;;) {
    for (int i = 0; i < 4; i++) {
      ...
      flow3r_bsp_display_send_fb(
          tildagon_fb +
              (TILDAGON_DISPLAY_WIDTH * TILDAGON_DISPLAY_HEIGHT * 2 / 4) * i,
          i);
...
static mp_obj_t start_display_flip_task() {
  xTaskCreatePinnedToCore(vADisplayFlip, "DisplayFlip", 2048, NULL,
                          tskIDLE_PRIORITY + 2, &display_flip_task, 0);

  return mp_const_none;
}
static MP_DEFINE_CONST_FUN_OBJ_0(start_display_flip_task_obj,
                                 start_display_flip_task);

static inline Ctx *tildagon_gfx_ctx(void) {
  if (tildagon_ctx == NULL) {
    tildagon_ctx = ctx_new_for_framebuffer(
        tildagon_fb, TILDAGON_DISPLAY_WIDTH, TILDAGON_DISPLAY_HEIGHT,
        TILDAGON_DISPLAY_WIDTH * 2, CTX_FORMAT_RGB565_BYTESWAPPED);
  }
  return tildagon_ctx;
}
...
static inline void tildagon_start_frame(Ctx *ctx) {
  ...
  ctx_save(ctx);
  ctx_identity(ctx);
  ctx_apply_transform(ctx, 1.0f, 0.0f, offset_x, 0.0f, 1.0f, offset_y, 0.0f,
                      0.0f, 1.0f);
}
...
static mp_obj_t all_sections_ready() {
  return (fb_sect_state[0] == FB_INVALID) && (fb_sect_state[1] == FB_INVALID) &&
...
static mp_obj_t hexagon(size_t n_args, const mp_obj_t *args) {
  ...
  // Rotate so point is at the top - the drawing code has the flat side at the
  // top
...
static const mp_rom_map_elem_t display_module_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_flip_sasppu_section),
     MP_ROM_PTR(&flip_sasppu_section_obj)},
    {MP_ROM_QSTR(MP_QSTR_start_display_flip_task),
     MP_ROM_PTR(&start_display_flip_task_obj)},
    {MP_ROM_QSTR(MP_QSTR_section_ready), MP_ROM_PTR(&section_ready_obj)},
    {MP_ROM_QSTR(MP_QSTR_all_sections_ready),
     MP_ROM_PTR(&all_sections_ready_obj)},
