...
Pull the firmware build image:
    ...
    docker pull matthewwilkes/esp_idf:5.4

(Or build it yourself, if you prefer):

    docker build . -t matthewwilkes/esp_idf:5.4
...
Then to build the images run:
    ...
    docker run -it --rm --env "TARGET=esp32s3" -v "$(pwd)"/:/firmware -u $UID -e HOME=/tmp matthewwilkes/esp_idf:5.4
...
Alternatively, to flash a badge:
    ...
    docker run -it --rm --device /dev/ttyACM0:/dev/ttyUSB0 --env "TARGET=esp32s3" -v "$(pwd)"/:/firmware -u $UID -e HOME=/tmp matthewwilkes/esp_idf:5.4 deploy
